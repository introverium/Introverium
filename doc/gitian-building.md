...
*Setup instructions for a Gitian build of Introveriumcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Introveriumcoin
...
- [Building Introveriumcoin Core](#building-introveriumcoin-core)
...
Clone the git repositories for introveriumcoin and Gitian.
...
in the introveriumcoin repository under 'Fetch and create inputs' to install sources which require
...
To build Introveriumcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the introveriumcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/introveriumcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/introveriumcoin-project/introveriumcoin
...
For example:
```bash
URL=https://github.com/thrasher-/introveriumcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit introveriumcoin=${COMMIT} --url introveriumcoin=${URL} ../introveriumcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit introveriumcoin=${COMMIT} --url introveriumcoin=${URL} ../introveriumcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit introveriumcoin=${COMMIT} --url introveriumcoin=${URL} ../introveriumcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the introveriumcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../introveriumcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/introveriumcoin
SIGPATH=/some/root/path/introveriumcoin-detached-sigs

./bin/gbuild --url introveriumcoin=${BTCPATH},signature=${SIGPATH} ../introveriumcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/introveriumcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/introveriumcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/introveriumcoin-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/introveriumcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
